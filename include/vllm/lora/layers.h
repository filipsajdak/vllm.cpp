...
//    collectives (the TP row is mid-flight), and a version that drops them would
//    only be correct at tp_size == 1 — a hollow port that no gate could catch.
//    The sharded classes therefore carry their SLICING rules (pure functions of
//    tp_rank/tp_size, and the part W2 owns) and inherit the unsharded apply,
//    which is exactly the upstream behaviour at tp_size == 1.
...
class BaseLinearLayerWithLoRA {
 ...
 protected:
  ...
  // input dim, row_parallel_linear.py:32-37).
