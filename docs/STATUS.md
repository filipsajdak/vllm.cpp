...
| LoRA / multi-LoRA adapters | In progress (W1+W2 CPU runtime), not yet usable end-to-end | Highest-demand missing feature. W1 landed the punica CPU brick (`LoRALayerWeights`, shrink/expand with `-1`-slot skip, `AddLoraLinear`); W2 landed packed adapters and the wrapped layer family (merged qkv/gate_up slices, TP slicing, embedding and logits LoRA). RUNTIME-VERIFIED on CPU vs double-precision references, RED-first: `test_punica_cpu` 6/6 + `test_lora_layers` 8/8 (476 assertions). Mapping metadata, adapter load, the LRU manager, the endpoints and the GPU kernels + model gate are W3-W7 in `.agents/specs/lora-adapter.md`. No model can be served with an adapter yet |
